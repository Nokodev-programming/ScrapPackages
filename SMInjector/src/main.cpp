...
int main(int argc, char** argv) {
	printf("SPInjector - Injector for ScrapPackages\n\n");
	printf("[SPInjector]: Starting up injector...\n");
	...
	wprintf(L"[SPInjector]: Game path: [%s]\n", game_path.c_str());
	wprintf(L"[SPInjector]: DLL path : [%s]\n", dll_path.c_str());
	
	if(game_path.empty()) {
		printf("[SPInjector]: Failed to find the game \"ScrapMechanic\"\n");
		return 0;
	}
	...
	if(startup(exe_exe, exe_dir, L"-dev", hProcess, hThread)) {
		...
		if (!InjectSetDllDirectory(hProcess, dir_path.c_str())) {
			printf("[SPInjector]: Failed to inject dll directory\n");
		...
		{
			printf("[SPInjector]: Injected dll directory.\n");
		}

		// Inject SMLibrary.dll
		if (!Inject(hProcess, dll_path.c_str())) {
			printf("[SPInjector]: Failed to inject SMLibrary.dll\n");
			TerminateProcess(hProcess, 0);
			CloseHandle(hProcess);
			return 0;
		}
		...
		{
			printf("[SPInjector]: Injected SMLibary\n");
		}
